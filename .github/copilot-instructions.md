# EcoTrack App: AI Coding Agent Instructions

**Project**: EcoTrack App – Carbon compliance and emission tracking platform
...
- **Angular 19+ Client**: Generated via AngularCLI; follows standard Angular Frontend App layering (Controller → Service → Model).

### Key Packages & Responsibilities

### Frontend Structure

- `src/app/`: Angular 16+ standalone components + shared modules
...
- **UI Framework**: Angular Material M3 with Material Icons
...
**App runs on**: Frontend `http://localhost:4200`, Frontend dev server proxies to backend.
...
- Liquibase: 4.24.0
- Angular: 19.2+ (check package.json for exact version)
